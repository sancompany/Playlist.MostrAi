...
**`erro` é durável.** Vem de uma tabela SQLite, não de memória. Se o app
falhar às 3h, reiniciar, e mandar heartbeat às 3h05, o erro ainda está lá —
sem isso o estado "Erro do player" nunca acenderia. É limpo quando uma
playlist é buscada do servidor com sucesso.
...
| `versaoMinimaBuild` | não | `versionCode` mínimo aceitável |
| `operacao` | não | ver seção 7 |
| `update` | não | `horasEntreTentativas` limitado a 1–72 |
...
- **Atômica.** Ou o corpo inteiro é válido e é aplicado, ou nada é escrito.
...
exatamente o arquivo que acabou de ser rejeitado.
...
- Faixas em `HH:MM`, minuto de início **inclusivo**, fim **exclusivo**.
- `"inicio": "22:00", "fim": "02:00"` cruza a meia-noite e é tratado como tal.
- Dia da semana sem faixa = fechado naquele dia.
- `feriados` sobrepõe o dia da semana. Lista vazia = fechado o dia inteiro.
...
5. Pede instalação via `PackageInstaller` **só entre itens**, nunca no meio
   de exibição paga.
6. Cancelamento → `DEFERRED` + janela de silêncio (padrão 6h).
...
| Kiosk | `kiosk/Kiosk.kt`, `kiosk/Watchdog.kt` |
